...
MyRootGui::MyRootGui(const TGWindow *p, int w, int h) : TGMainFrame(p, w, h)
{
    ...
    fileDir = env->GetValue("fileDir", fileDir);
    pedDir  = env->GetValue("pedDir",  pedDir);
    ...
    fFileLabel1->MoveResize(120, 520, 900, 23);
    ...
    fOutputLabel1->MoveResize(x0, y0, 220, 140);
...
void MyRootGui::Analysis()
{
    ...
    if(retval == 2) gMyRootClass->Analysis(filePath);
    else            gMyRootClass->AnalysisDir(fileDir);
