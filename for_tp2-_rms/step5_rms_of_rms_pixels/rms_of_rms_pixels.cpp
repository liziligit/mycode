...
RadFile = argv[1];
char *RadiationType;
RadiationType = argv[2];
// char *store_file_name;
// store_file_name = argv[3];

char Radfn[100];
sprintf(Radfn,"%s.txt", RadFile);
...
FILE *n1=fopen(Radfn,"r+");

if (!n1) {
      std::cout << "Unable to open RadFile file "  <<std::endl;
      return -1; 
   }  

ofstream n2("./rd_nord_rms_of_rms_pixels.txt",ios::app);
...
while(fscanf(n1,"%lf%lf%lf",&noise[0],&noise[1],&noise[2])!=EOF)
//for(i = 0; i<=41472;i++){
...
nois[j]->GetYaxis()->SetTitle("100 events rms' rms of pixel");
nois[j]->GetXaxis()->SetLimits(0,5200);
//nois[j]->GetYaxis()->SetLimits(0,40);
nois[j]->GetYaxis()->SetRangeUser(0,0.12);
...
n2<<"\n";
//c1->Draw();
c1->SaveAs(TString::Format("./%s_rms_of_rms_pixels.png",RadiationType));
